(function($) {
    AOS.init();
    "use strict"
    $('.title-text .letters').each(function() {
        $(this).html($(this).text().replace(/([^\x00-\x80]|\w)/g, "<span class='letter'>$&</span>"));
    });
    $('.nav-items:not(.wp)').each(function() {
        $(this).html($(this).text().replace(/([^\x00-\x80]|\w|&)/g, "<span class='letter'>$&</span>"));
    });
    ...
    $(window).on('load', function() {
        ...
        $('#' + day + '.schedule').addClass('show');

    });

    var animeLetter = function() {
        anime.timeline()
            .add({
                targets: '.nav-items:hover>.letter',
                translateY: [0, -20],
                opacity: [1, 0],
                easing: "easeInSine",
                duration: 300,
    ...
    $('.nav-items').mouseenter(animeLetter);

    $('#preview .mob-selector').on('click', function() {
        $('#preview .selector').toggleClass('open');
        $('#preview .mob-selector .mob-mask').toggleClass('open');
    });
    $('#preview .type-select').on('click', function() {
        $('#preview .selector').removeClass('open');
        $('#preview .mob-selector .mob-mask').removeClass('open');
    });

    $(window).on('scroll', function() {
        $(this).scrollTop() > 700 ? $('#back-to-top').fadeIn() : $('#back-to-top').fadeOut();
        $(this).scrollTop() > 150 ? $('#sponsorsnpartners .btn-back').fadeIn() : $('#sponsorsnpartners .btn-back').fadeOut();
        $(this).scrollTop() > $(document).height() - $(window).height() * 1.6 ? $('#preview .btn-back').fadeOut(500) : $('#preview .btn-back').fadeIn(500, 'swing');
    });
    ...
    $('.day').on('click', function() {
        ...
        $('#' + thisId + '.day').addClass('show');
        $('#' + thisId + '.schedule').addClass('show');
    });
